...
6. Open Studio → Style library → **Install and use L8R Editorial v2**. Alternatively run `node --conditions=react-server --import tsx scripts/studio-install-style.ts --confirm`. This imports 40 supplied images privately; only five are eligible anchors. Installation is idempotent.
